...
cxy=0; cxz=0; cyz=0; cx=0; cy=0; cz=0

cxx(1,1) = 1.0_my_real
cyy(1,1) = 1.0_my_real
czz(1,1) = 1.0_my_real
c(1,1) = 0.5_my_real*prob_param*(x*x+3.0_my_real*y*y+2.0_my_real*z*z)
rs(1) = 1.0_my_real
...
real (my_real) :: xmax,ymax,tmax,d,sq3,sq2
!----------------------------------------------------
! Begin executable code
...
case(1)
   ...
   trues = exp(-d*(x*x+sq3*y*y+sq2*z*z))
...
case(1)
   ...
   truexs = exp(-d*(x*x+sq3*y*y+sq2*z*z))
   truexs = -2*d*x*truexs
...
case(1)
   ...
   trueys = exp(-d*(x*x+sq3*y*y+sq2*z*z))
...
case(1)
   ...
   truezs = exp(-d*(x*x+sq3*y*y+sq2*z*z))
   truezs = -2*sq2*d*z*truezs

case default

   truezs = huge(0.0_my_real)
...
use phaml
real(my_real), intent(in) :: x(3),y(3),z(3)
