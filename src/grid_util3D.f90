...
public allocate_grid, &        ! not thread safe
       ...
       face_exact, &           ! thread safe if vertex and edge not changing
       elem_exact, &           ! thread safe if vert, edge and face not changing
...
allocate(grid%element(11*nvert),grid%edge(8*nvert),grid%vertex(nvert), &
         grid%face(17*nvert),stat=astat1)
...
if (ierr == ALLOC_FAILED) then
   deallocate(grid%element,grid%edge,grid%vertex,grid%errest_energy, &
              ...
              grid%errest_eigenvalue,grid%face,stat=dstat)
   return
endif
call hash_table_init(grid%face_hash,size(grid%face))
if (ierr == ALLOC_FAILED) then
...
allocate(grid%face_type(size(grid%face),grid%system_size),stat=astat1)
if (astat1 /= 0) then
   ierr = ALLOC_FAILED
   call fatal("memory allocation failed in allocate_grid")
   deallocate(grid%face_type,stat=dstat)
   call hash_table_destroy(grid%face_hash)
   deallocate(grid%element_errind,stat=dstat)
   deallocate(grid%head_level_elem,stat=dstat)
   call hash_table_destroy(grid%elem_hash)
...
if (astat1 /= 0) then
   ...
   deallocate(grid%element,grid%edge,grid%vertex,grid%errest_energy, &
              grid%errest_Linf,grid%errest_L2, &
              grid%errest_eigenvalue,grid%face,grid%face_type,stat=dstat)
   return
endif
...
if (associated(grid%face_type)) deallocate(grid%face_type,stat=dstat)
do i=1,grid%biggest_face
   if (associated(grid%face(i)%solution)) deallocate(grid%face(i)%solution)
   if (associated(grid%face(i)%exact)) deallocate(grid%face(i)%exact)
   if (associated(grid%face(i)%oldsoln)) deallocate(grid%face(i)%oldsoln)
end do
...
deallocate(grid%element, grid%edge, grid%vertex, grid%head_level_elem, &
           ...
           grid%errest_L2, grid%errest_eigenvalue, grid%face, stat=dstat)
...
integer :: i, ncopy, ncopyss, ncopyev, astat, nsolut, neq_edge, neq_bubble, &
           neq_copy, deg, lev, elem, edge, face, neq_face
...
if (size(grid%vertex_solution,2) /= system_size .or. &
   ...
   temp3(1:grid%biggest_vert,1:ncopyss,1:ncopyev) = &
      grid%vertex_solution(1:grid%biggest_vert,1:ncopyss,1:ncopyev)
...
! faces
      ...
      do i=1,FACES_PER_ELEMENT
         ...
         if (degree > 0) then
            deg = degree
            if (grid%face(face)%degree > 0) then
               grid%dof = grid%dof - &
                       count(grid%face_type(face,:)/=PERIODIC_SLAVE)* &
                       face_dof(grid%face(face)%degree)
               if (grid%element(grid%face(face)%assoc_elem)%iown) then
                  grid%dof_own = grid%dof_own - &
                       count(grid%face_type(face,:)/=PERIODIC_SLAVE)* &
                       face_dof(grid%face(face)%degree)
               endif
            endif
            grid%face(face)%degree = degree
            grid%dof = grid%dof + &
                       count(grid%face_type(face,:)/=PERIODIC_SLAVE)* &
                       face_dof(grid%face(face)%degree)
            if (grid%element(grid%face(face)%assoc_elem)%iown) then
            ...
            deg = grid%face(face)%degree
         endif
         neq_face = face_dof(deg)
         if (deg <= 1) then
            if (associated(grid%face(face)%solution)) then
               deallocate(grid%face(face)%solution)
            endif
            if (associated(grid%face(face)%exact)) then
               deallocate(grid%face(face)%exact)
            endif
            nullify(grid%face(face)%solution,grid%face(face)%exact)
         else
            if (associated(grid%face(face)%solution)) then
               if (size(grid%face(face)%solution,dim=1) == neq_face .and. &
                   size(grid%face(face)%solution,dim=2) == system_size .and. &
                   size(grid%face(face)%solution,dim=3) == neval) cycle
            endif
            if (associated(grid%face(face)%solution)) then
               neq_copy = min(neq_face,size(grid%face(face)%solution,dim=1))
            else
               neq_copy = 0
            endif
            allocate(temp3(neq_face,system_size,neval),stat=astat)
            if (astat /= 0) then
               ierr = ALLOC_FAILED
               call fatal("memory allocation failed in realloc_solution")
               return
            ...
            if (neq_copy > 0) then
               temp3(1:neq_copy,1:ncopyss,1:ncopyev) = &
                  grid%face(face)%solution(1:neq_copy,1:ncopyss,1:ncopyev)
            endif
            if (associated(grid%face(face)%solution)) then
               deallocate(grid%face(face)%solution,stat=astat)
            endif
            grid%face(face)%solution => temp3
            nullify(temp3)
            if (grid%have_true) then
               if (associated(grid%face(face)%exact)) then
                  neq_copy = min(neq_face,size(grid%face(face)%exact,dim=1))
               else
                  neq_copy = 0
               endif
               allocate(temp3(neq_face,system_size,neval),stat=astat)
               ...
               if (neq_copy > 0) then
                  temp3(1:neq_copy,1:ncopyss,1:ncopyev) = &
                     grid%face(face)%exact(1:neq_copy,1:ncopyss,1:ncopyev)
               endif
               if (associated(grid%face(face)%exact)) then
                  deallocate(grid%face(face)%exact,stat=astat)
               endif
               grid%face(face)%exact => temp3
               nullify(temp3)
            endif
         endif
      end do
...
! TEMP 3D
   if (grid%element(elem)%degree /= 1) then
      call fatal("have not finished 3D version of reset_dirich_exact")
      stop
   endif
! end TEMP 3D
   do while (elem /= END_OF_LIST)
...
integer :: i, astat
type(bvert_type), pointer :: old_bvert, new_bvert
!----------------------------------------------------
! Begin executable code
...
if (associated(old_grid%element)) then
   ...
   do i=1,size(old_grid%element)
      ...
      new_grid%element(i)%neighbor_hint = old_grid%element(i)%neighbor_hint
      ...
      new_grid%element(i)%face = old_grid%element(i)%face
      ...
      new_grid%element(i)%refinement_edge = old_grid%element(i)%refinement_edge
      ...
      new_grid%element(i)%type = old_grid%element(i)%type
   end do
else
...
if (associated(old_grid%face)) then
   allocate(new_grid%face(size(old_grid%face)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   do i=1,size(old_grid%face)
      new_grid%face(i)%gid = old_grid%face(i)%gid
      new_grid%face(i)%edge = old_grid%face(i)%edge
      new_grid%face(i)%vertex = old_grid%face(i)%vertex
      new_grid%face(i)%child = old_grid%face(i)%child
      new_grid%face(i)%bmark = old_grid%face(i)%bmark
      new_grid%face(i)%degree = old_grid%face(i)%degree
      new_grid%face(i)%assoc_elem = old_grid%face(i)%assoc_elem
      new_grid%face(i)%marked_edge = old_grid%face(i)%marked_edge
      new_grid%face(i)%tags = old_grid%face(i)%tags
      if (associated(old_grid%face(i)%solution)) then
         allocate(new_grid%face(i)%solution( &
                  size(old_grid%face(i)%solution,dim=1), &
                  size(old_grid%face(i)%solution,dim=2), &
                  size(old_grid%face(i)%solution,dim=3)),stat=astat)
         if (astat /= 0) then
            ierr = ALLOC_FAILED
            call fatal("memory allocation failed in copy_grid")
            return
         endif
         new_grid%face(i)%solution = old_grid%face(i)%solution
      else
         nullify(new_grid%face(i)%solution)
      endif
      if (associated(old_grid%face(i)%exact)) then
         allocate(new_grid%face(i)%exact( &
                  size(old_grid%face(i)%exact,dim=1), &
                  size(old_grid%face(i)%exact,dim=2), &
                  size(old_grid%face(i)%exact,dim=3)),stat=astat)
         if (astat /= 0) then
            ierr = ALLOC_FAILED
            call fatal("memory allocation failed in copy_grid")
            return
         endif
         new_grid%face(i)%exact = old_grid%face(i)%exact
      else
         nullify(new_grid%face(i)%exact)
      endif
      if (associated(old_grid%face(i)%oldsoln)) then
         allocate(new_grid%face(i)%oldsoln( &
                  size(old_grid%face(i)%oldsoln,dim=1), &
                  size(old_grid%face(i)%oldsoln,dim=2), &
                  size(old_grid%face(i)%oldsoln,dim=3)),stat=astat)
         new_grid%face(i)%oldsoln = old_grid%face(i)%oldsoln
         if (astat /= 0) then
            ierr = ALLOC_FAILED
            call fatal("memory allocation failed in copy_grid")
            return
         ...
         nullify(new_grid%face(i)%oldsoln)
      endif
      new_grid%face(i)%next = old_grid%face(i)%next
   end do
else
   nullify(new_grid%face)
endif
...
if (associated(old_grid%edge)) then
   ...
   do i=1,size(old_grid%edge)
      ...
      new_grid%edge(i)%child = old_grid%edge(i)%child
...
if (associated(old_grid%vertex)) then
   ...
   do i=1,size(old_grid%vertex)
      ...
      if (i <= old_grid%biggest_vert) then
         if (associated(old_grid%vertex(i)%boundary_vertex)) then
            allocate(new_grid%vertex(i)%boundary_vertex,stat=astat)
            if (astat /= 0) then
               ierr = ALLOC_FAILED
               call fatal("memory allocation failed in copy_grid")
               return
            endif
            new_bvert => new_grid%vertex(i)%boundary_vertex
            old_bvert => old_grid%vertex(i)%boundary_vertex
            do while (associated(old_bvert))
               new_bvert = old_bvert
               nullify(new_bvert%next)
               old_bvert => old_bvert%next
               if (associated(old_bvert)) then
                  allocate(new_bvert%next,stat=astat)
                  if (astat /= 0) then
                     ierr = ALLOC_FAILED
                     call fatal("memory allocation failed in copy_grid")
                     return
                  endif
                  new_bvert => new_bvert%next
               endif
            end do
         else
            nullify(new_grid%vertex(i)%boundary_vertex)
         endif
      endif
   end do
else
...
if (associated(old_grid%face_type)) then
   allocate(new_grid%face_type( &
            size(old_grid%face_type,dim=1), &
            size(old_grid%face_type,dim=2)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   new_grid%face_type = old_grid%face_type
else
...
if (associated(old_grid%point)) then
   allocate(new_grid%point(size(old_grid%point)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   new_grid%point(1:old_grid%npoint) = old_grid%point(1:old_grid%npoint)
else
   nullify(new_grid%point)
endif
if (associated(old_grid%line)) then
   allocate(new_grid%line(size(old_grid%line)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   new_grid%line(1:old_grid%nline) = old_grid%line(1:old_grid%nline)
   do i=1,old_grid%nline
      if (associated(old_grid%line(i)%point)) then
         allocate(new_grid%line(i)%point(size(old_grid%line(i)%point)), &
                  stat=astat)
         if (astat /= 0) then
            ierr = ALLOC_FAILED
            call fatal("memory allocation failed in copy_grid")
         ...
         new_grid%line(i)%point = old_grid%line(i)%point
      else
         nullify(new_grid%line(i)%point)
      endif
   end do
else
   nullify(new_grid%line)
endif
if (associated(old_grid%lineloop)) then
   allocate(new_grid%lineloop(size(old_grid%lineloop)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   new_grid%lineloop(1:old_grid%nlineloop) = &
                                        old_grid%lineloop(1:old_grid%nlineloop)
   do i=1,old_grid%nlineloop
      if (associated(old_grid%lineloop(i)%line)) then
         allocate(new_grid%lineloop(i)%line(size(old_grid%lineloop(i)%line)), &
                  stat=astat)
         if (astat /= 0) then
            ierr = ALLOC_FAILED
            call fatal("memory allocation failed in copy_grid")
         ...
         new_grid%lineloop(i)%line = old_grid%lineloop(i)%line
      else
         nullify(new_grid%lineloop(i)%line)
      endif
   end do
else
   nullify(new_grid%lineloop)
endif
if (associated(old_grid%surface)) then
   allocate(new_grid%surface(size(old_grid%surface)),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
      call fatal("memory allocation failed in copy_grid")
      return
   endif
   new_grid%surface(1:old_grid%nsurface) = old_grid%surface(1:old_grid%nsurface)
else
   nullify(new_grid%surface)
endif
new_grid%npoint = old_grid%npoint
new_grid%nline = old_grid%nline
new_grid%nlineloop = old_grid%nlineloop
new_grid%nsurface = old_grid%nsurface
...
integer :: lev, elem, astat, i, edge, face
!----------------------------------------------------
! Begin executable code
...
! if the element is a leaf and the face solution is allocated, make sure the
! face oldsoln is the same size and copy solution to it

               if (associated(grid%face(face)%oldsoln)) then
                  if (size(grid%face(face)%oldsoln,dim=1) /= &
                      size(grid%face(face)%solution,dim=1) .or. &
                      size(grid%face(face)%oldsoln,dim=2) /= &
                      size(grid%face(face)%solution,dim=2) .or. &
                      size(grid%face(face)%oldsoln,dim=3) /= &
                      size(grid%face(face)%solution,dim=3)) then
                     deallocate(grid%face(face)%oldsoln,stat=astat)
                  endif
               endif
               if (.not. associated(grid%face(face)%oldsoln)) then
                  allocate(grid%face(face)%oldsoln( &
                             size(grid%face(face)%solution,dim=1), &
                             size(grid%face(face)%solution,dim=2), &
                             size(grid%face(face)%solution,dim=3)),stat=astat)
                  if (astat /= 0) then
                     ierr = ALLOC_FAILED
                     call fatal("memory allocation failed in copy_old")
                     stop
               ...
               grid%face(face)%oldsoln = grid%face(face)%solution

            else

! if element is a leaf and face does not have a solution, make sure it does not
! have oldsoln either
...
! if element is not a leaf, don't change the face because the neighbor might
! be a leaf and want the face set, and it won't cause any damage to leave it
...
! TEMP 3D
call fatal("have not finished 3D version of edge_exact")
stop
...
!          ----------
subroutine face_exact(grid,face,sysrank,what)
!          ----------

!----------------------------------------------------
! This routine sets "exact" solutions on a face.  what indicates what is
! being set, and can be "d" for Dirichlet boundary conditions, "i" for
! initial conditions, or "t" for the true solution.
! It assumes the coefficients for the bases at the vertices and edges of
! face are already set, and computes the coefficients of the high order
! bases on that face as a least squares fit to the function.
! For multiple eigenvalue problems, all eigenfunctions are set.  It is
! assumed that they all satisfy the same boundary conditions (bconds is
! defined for component=1..system_size) but have different initial conditions
! and true solutions (iconds and trues are defined for component=1..system_size
...
type(grid_type), intent(inout) :: grid
integer, intent(in) :: face, sysrank
character(len=1), intent(in) :: what
!----------------------------------------------------
! Local variables:
...
if (grid%face(face)%degree < 3) return

! TEMP 3D
call fatal("have not written face_exact")
stop
...
! It assumes the coefficients for the bases at the vertices, edges and faces
...
integer :: i, j, k, nqp, nev, ss, jerr, astat, n, degree, nbasis, isub, &
           deg(EDGES_PER_ELEMENT+FACES_PER_ELEMENT+1)
real(my_real) :: xv(VERTICES_PER_ELEMENT),yv(VERTICES_PER_ELEMENT)
real(my_real), pointer :: weight(:), xq(:), yq(:), zq(:)
...
! TEMP 3D
call fatal("have not finished 3D version of elem_exact")
stop
...
do i=1,FACES_PER_ELEMENT
   deg(EDGES_PER_ELEMENT+i) = grid%face(grid%element(elem)%face(i))%degree
end do
deg(EDGES_PER_ELEMENT+FACES_PER_ELEMENT+1) = grid%element(elem)%degree
nbasis = nbasis + element_dof(deg(EDGES_PER_ELEMENT+FACES_PER_ELEMENT+1))
...
!          -----------------
subroutine old_get_edge_elements(grid,edge,elements)
!          -----------------

!----------------------------------------------------
! This routine returns all the elements that share edge.  If the edge is
! on the boundary, the first and last elements will be on the boundary.
! Otherwise, the first element is the associated element and the second and
! last share a face with it.
!----------------------------------------------------
...
integer :: current_elem, nelem, previous_elem, pass, i, j, &
           neigh(NEIGHBORS_PER_ELEMENT), nfound, sharer(2), end_part_1, &
           other_initial_neigh, astat
integer :: element_list(100) ! surely there can't be 100 elements sharing the edge
!----------------------------------------------------
! Begin executable code

! start with the associated element

current_elem = grid%edge(edge)%assoc_elem
nelem = 1
element_list(nelem) = current_elem
previous_elem = -1

! loop to move from the current element to a neighbor until with either hit
! the boundary or return to the starting element.  If we hit a boundary, make
! a second pass starting at the other initial neighbor
...
! get the neighbor elements
      ...
      neigh = get_neighbors(current_elem,grid)

! find the neighbors that share the edge

      nfound = 0
      do i=1,NEIGHBORS_PER_ELEMENT
         if (any(grid%element(neigh(i))%edge == edge)) then
            nfound = nfound + 1
            if (nfound > 2) then
               ierr = PHAML_INTERNAL_ERROR
               call fatal("get_edge_elements: found too many neighbors sharing an edge")
               stop
            endif
            sharer(nfound) = neigh(i)
         endif
      end do

! if the number we found is less than 2, the edge had better be boundary

      if (nfound < 2 .and. grid%edge_type(edge,1) == INTERIOR) then
         ierr = PHAML_INTERNAL_ERROR
         call fatal("get_edge_elements: found less than two neighbors sharing the internal edge")
         stop
      endif

      select case (nfound)

! if nfound is 0, then this is a boundary edge on only one element, so done

      case (0)
         if (nelem /= 1) then
            ierr = PHAML_INTERNAL_ERROR
            call fatal("get_edge_elements: found no neighbors but nelem > 1")
            stop
         endif
         end_part_1 = nelem
         exit outer

! if nfound is 1, then we have hit the boundary.  The one that was found
! should be the previous, unless this is the first element.

      case (1)
         if (nelem > 1 .and. sharer(1) /= previous_elem) then
            ierr = PHAML_INTERNAL_ERROR
            call fatal("get_edge_elements: found 1 sharer, but not previous")
            stop
         endif
         if (pass == 1) then
            end_part_1 = nelem
            nelem = nelem + 1
            if (nelem == 2) then
               element_list(nelem) = sharer(1)
            else
               element_list(nelem) = other_initial_neigh
            endif
            previous_elem = current_elem
            current_elem = element_list(nelem)
         endif
         exit

! if nfound is 2, pick the one that is not the previous element, and check
! for getting back to the original element

      case (2)
         nelem = nelem + 1
         if (sharer(1) /= previous_elem) then
            element_list(nelem) = sharer(1)
            if (nelem == 2) other_initial_neigh = sharer(2)
         else
            element_list(nelem) = sharer(2)
            if (nelem == 2) other_initial_neigh = sharer(1)
         endif
         previous_elem = current_elem
         current_elem = element_list(nelem)
         if (current_elem == element_list(1)) then
            nelem = nelem - 1
            end_part_1 = nelem
            exit outer
         endif
...
! copy the element list to the return variable, starting at the second
! boundary and ending at the first boundary, or starting at the associated elem

allocate(elements(nelem),stat=astat)
if (astat /= 0) then
   ierr = ALLOC_FAILED
   call fatal("allocation failed in get_edge_elements")
   stop
endif
j = 1
do i=nelem, end_part_1+1, -1
   elements(j) = element_list(i)
   j = j+1
end do
do i=1,end_part_1
   elements(j) = element_list(i)
...
end subroutine old_get_edge_elements
...
! If edge is a periodic slave, then one or more nexts give the master.

if (is_periodic_edge_slave(edge,grid)) then
   matching_periodic_edge = grid%edge(edge)%next
   do while (is_periodic_edge_slave(matching_periodic_edge,grid))
      matching_periodic_edge = grid%edge(matching_periodic_edge)%next
   end do

! Otherwise, next gives a matching edge

elseif (is_periodic_edge_master(edge,grid)) then
   matching_periodic_edge = grid%edge(edge)%next
...
if (present(comp)) then
   is_periodic_edge=grid%edge_type(edge,comp)==PERIODIC_MASTER     .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_DIR .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_NAT .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_MIX .or. &
                    grid%edge_type(edge,comp)==PERIODIC_SLAVE      .or. &
                    grid%edge_type(edge,comp)==PERIODIC_SLAVE_DIR  .or. &
                    grid%edge_type(edge,comp)==PERIODIC_SLAVE_NAT  .or. &
                    grid%edge_type(edge,comp)==PERIODIC_SLAVE_MIX
else
   is_periodic_edge=any(grid%edge_type(edge,:)==PERIODIC_MASTER)     .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_DIR) .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_NAT) .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_MIX) .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_SLAVE)      .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_SLAVE_DIR)  .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_SLAVE_NAT)  .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_SLAVE_MIX)
endif
...
!----------------------------------------------------
! This routine returns true if the given face is periodic.
! If comp is present, it only checks component comp; otherwise it is periodic
! if any component is periodic.
!----------------------------------------------------
...
if (present(comp)) then
   is_periodic_face=grid%face_type(face,comp)==PERIODIC_MASTER     .or. &
                    grid%face_type(face,comp)==PERIODIC_SLAVE
else
   is_periodic_face=any(grid%face_type(face,:)==PERIODIC_MASTER)     .or. &
                    any(grid%face_type(face,:)==PERIODIC_SLAVE)
endif
...
if (present(comp)) then
   is_periodic_edge_master = &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER     .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_DIR .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_NAT .or. &
                    grid%edge_type(edge,comp)==PERIODIC_MASTER_MIX
else
   is_periodic_edge_master = &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER)     .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_DIR) .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_NAT) .or. &
                    any(grid%edge_type(edge,:)==PERIODIC_MASTER_MIX)
endif
...
!----------------------------------------------------
! This routine returns true if the given face is periodic and the master.
! If comp is present, it only checks component comp; otherwise it is periodic
! if any component is periodic.
!----------------------------------------------------
...
if (present(comp)) then
   is_periodic_face_master = grid%face_type(face,comp)==PERIODIC_MASTER
else
   is_periodic_face_master = any(grid%face_type(face,:)==PERIODIC_MASTER)
endif
...
!----------------------------------------------------
! This routine returns true if the given face is periodic and a slave.
! If comp is present, it only checks component comp; otherwise it is periodic
! if any component is periodic.
!----------------------------------------------------
...
if (present(comp)) then
   is_periodic_face_slave = grid%face_type(face,comp)==PERIODIC_SLAVE
else
   is_periodic_face_slave = any(grid%face_type(face,:)==PERIODIC_SLAVE)
endif
...
!        -------------
function get_neighbors(lid,grid,missing_OK)
!        -------------
...
integer :: parent_lid, child_lid(MAX_CHILD)
type(hash_key) :: gid, parent, child1, gid_neighbor
type(hash_key) :: children(MAX_CHILD)
integer :: allc(MAX_CHILD)
integer :: i, j, k, vert, other_vert(1), edge, other_edge(EDGES_PER_ELEMENT), &
           lastone, neigh_lid, periodic_face
logical :: seen_face, missing_not_OK
type(hash_key) :: neigh_child(MAX_CHILD)
type(hash_key) :: possible_neighbor(NEIGHBORS_PER_ELEMENT)
!----------------------------------------------------
! Begin executable code

if (present(missing_OK)) then
   missing_not_OK = .not. missing_OK
else
   missing_not_OK = .true.
endif

! start with initial neighbors on level 1 or neighbor hint

if (grid%element(lid)%level == 1) then
   do i=1,NEIGHBORS_PER_ELEMENT
      if (grid%initial_neighbor(i,lid) == BOUNDARY) then
         possible_neighbor(i) = BOUNDARY
      else
         possible_neighbor(i) = grid%element(grid%initial_neighbor(i,lid))%gid
      endif
   end do
else
   possible_neighbor = grid%element(lid)%neighbor_hint
endif

! for each non-boundary neighbor

do i=1,NEIGHBORS_PER_ELEMENT
   if (possible_neighbor(i) == BOUNDARY) then
      get_neighbors(i) = BOUNDARY
      cycle
   endif

! if the element doesn't exist, then move up the ancestors until you find one

   neigh_lid = hash_decode_key(possible_neighbor(i),grid%elem_hash)
   if (neigh_lid == HASH_NOT_FOUND) then
      do while (neigh_lid == HASH_NOT_FOUND)
         possible_neighbor(i) = possible_neighbor(i)/MAX_CHILD
         neigh_lid = hash_decode_key(possible_neighbor(i),grid%elem_hash)
      end do
      get_neighbors(i) = neigh_lid

   else

! if the element has children, move down the child list identifying which child
! shares the face until a leaf is found

      get_neighbors(i) = neigh_lid
      lastone = get_neighbors(i)
      seen_face = element_contains_face(lastone,grid%element(lid)%face(i),grid)
      call search_children(get_neighbors(i),grid%element(lid)%face(i), &
                           seen_face,lastone,grid)
      if (.not. seen_face .and. missing_not_OK) then
         ierr = PHAML_INTERNAL_ERROR
         call fatal("failed to find all neighbors in get_neighbors")
         read *
         stop
      endif
      get_neighbors(i) = lastone

   endif

! reset the neighbor hint to the actual neighbor

   grid%element(lid)%neighbor_hint(i) = grid%element(get_neighbors(i))%gid
...
integer, parameter :: MAX_NEIGH = 100
integer :: nelem, loc_elements(MAX_NEIGH), neighbors(NEIGHBORS_PER_ELEMENT), &
           neigh, ineigh, i, j, next, astat, periodic_edge, child(MAX_CHILD)
logical :: loc_periodic, fixed_it
integer, pointer :: periodic_elements(:)
!----------------------------------------------------
! Begin executable code
...
! get the neighbors of the next element in the list
! I think it is OK to have missing neighbors.  This can happen during
! remove_hanging_nodes, but the missing neighbor will not be one that shares
! this edge.

   neighbors = get_neighbors(loc_elements(next),grid,missing_OK=.true.)
   next = next + 1
...
if (loc_periodic) then
   if (is_periodic_edge(edge,grid)) then
      ...
      do while (periodic_edge /= edge)
         call get_edge_elements(grid,periodic_edge,periodic_elements)
         call merge_periodic_edge
         deallocate(periodic_elements)
         periodic_edge = grid%edge(periodic_edge)%next
      end do

   endif
endif

! check each element for a child that contains the edge, and replace it if
! found (normally all the elements will be leaves, but remove_hanging_nodes
! calls this with a non-leaf edge and can fail without this check)
! If it is replaced, make sure it didn't create a duplicate.
...
do while (i<nelem)
   ...
   do while (fixed_it)
      fixed_it = .false.
      child = get_child_lid(grid%element(loc_elements(i))%gid,ALL_CHILDREN, &
                            grid%elem_hash)
      if (child(1) /= NO_CHILD) then
         do j=1,MAX_CHILD
            if (any(grid%element(child(j))%edge == edge)) then
               loc_elements(i) = child(j)
               fixed_it = .true.
               exit
            endif
         end do
      ...
      if (fixed_it) then
         if (any(loc_elements(1:i-1)==loc_elements(i)) .or. &
             any(loc_elements(i+1:nelem)==loc_elements(i))) then
            loc_elements(i:nelem-1) = loc_elements(i+1:nelem)
            nelem = nelem - 1
         endif
      endif
   end do
...
!                    -----------------
recursive subroutine get_face_elements(grid,face,elements,include_periodic)
!                    -----------------

!----------------------------------------------------
! This routine returns a list of all the elements around face face.
! If include_periodic is present and true, it also includes elements
! around periodic mates.
!----------------------------------------------------
...
integer, parameter :: MAX_NEIGH = 100
integer :: nelem, loc_elements(MAX_NEIGH), neighbors(NEIGHBORS_PER_ELEMENT), &
           neigh, ineigh, i, next, astat, periodic_face
logical :: loc_periodic
integer, pointer :: periodic_elements(:)
!----------------------------------------------------
! Begin executable code
...
loc_elements(1) = grid%face(face)%assoc_elem
nelem = 1
next = 1
...
! if any face is face, add it to the list

      if (any(grid%element(neigh)%face == face)) then
         nelem = nelem + 1
         if (nelem > MAX_NEIGH) then
            ierr = PHAML_INTERNAL_ERROR
            call fatal("too many elements in get_face_elements")
            stop
         endif
         loc_elements(nelem) = neigh
      endif
...
! if the face is periodic and periodic elements are to be included,
! include the elements around the periodic mate

if (loc_periodic) then
   if (is_periodic_face(face,grid)) then

      periodic_face = grid%face(face)%next
      call get_face_elements(grid,periodic_face,periodic_elements)
      call merge_periodic_face
      deallocate(periodic_elements)
...
if (astat /= 0) then
   ierr = ALLOC_FAILED
   call fatal("memory allocation failed in get_face_elements")
   stop
endif
...
subroutine merge_periodic_face
integer :: ii
do ii=1,size(periodic_elements)
   if (.not. any(loc_elements(1:nelem) == periodic_elements(ii))) then
      nelem = nelem + 1
...
end subroutine merge_periodic_face

end subroutine get_face_elements

!                    ---------------
recursive subroutine search_children(neigh,face,seen_face,lastone,grid)
!                    ---------------

!----------------------------------------------------
! This routine searches the refinement tree below neigh to find the last
! element that contains face.  The result is returned in lastone.
!----------------------------------------------------

!----------------------------------------------------
! Dummy arguments

integer, intent(in) :: neigh, face
logical, intent(inout) :: seen_face
integer, intent(inout) :: lastone
type(grid_type), intent(in) :: grid

!----------------------------------------------------
! Local variables:

integer :: neigh_child(MAX_CHILD)

!----------------------------------------------------
! Begin executable code

! get the children of the neighbor

neigh_child = get_child_lid(grid%element(neigh)%gid,ALL_CHILDREN,grid%elem_hash)

! if there are no children, we are done

if (neigh_child(1) == NO_CHILD) return

! if either child contains face, put it in lastone

if (element_contains_face(neigh_child(1),face,grid)) then
   lastone = neigh_child(1)
   seen_face = .true.
elseif (element_contains_face(neigh_child(2),face,grid)) then
   lastone = neigh_child(2)
   seen_face = .true.

! if neither child contains face and we have seen face in an ancestor, their
! descendants can't contain face either, so quit

elseif (seen_face) then
   return
...
call search_children(neigh_child(1),face,seen_face,lastone,grid)
call search_children(neigh_child(2),face,seen_face,lastone,grid)

end subroutine search_children

!        ---------------------
function element_contains_face(elem,face,grid)
!        ---------------------

!----------------------------------------------------
! This routine returns true if element elem contains face face or
! a periodic mate of face
!----------------------------------------------------

!----------------------------------------------------
! Dummy arguments

integer, intent(in) :: elem, face
type(grid_type), intent(in) :: grid
logical :: element_contains_face
!----------------------------------------------------
! Local variables:

integer :: periodic_face
!----------------------------------------------------
! Begin executable code

element_contains_face = any(grid%element(elem)%face == face)
periodic_face = grid%face(face)%next
do while (periodic_face /= face)
   element_contains_face = element_contains_face .or. &
                           any(grid%element(elem)%face == periodic_face)
   periodic_face = grid%face(periodic_face)%next
end do

end function element_contains_face
...
integer :: i, lev, vert, astat, ivert, elem
logical(small_logical) :: visited_vert(grid%biggest_vert)
!----------------------------------------------------
! Begin executable code

if (present(z)) then
   allocate(x(grid%nvert_own),y(grid%nvert_own),z(grid%nvert_own), &
            u(grid%nvert_own),stat=astat)
   if (astat /= 0) then
      ierr = ALLOC_FAILED
   ...
   ierr = PHAML_INTERNAL_ERROR
   call fatal("get_vertices_and_solution: z not present in call to 3D code")
   stop
endif
...
do while (elem /= END_OF_LIST)
   ...
   if (any(cxx /= 1.0_my_real) .or. any(cyy /= 1.0_my_real) .or. &
       any(czz /= 1.0_my_real) .or. any(c   /= 0.0_my_real)) then
      laplacian = .false.
...
!----------------------------------------------------
! In 2D this routine checks to see if all level 1 triangles are isoceles right
! triangles with the third vertex at the right angle.  In 3D it returns false.
!----------------------------------------------------
...
!        ------------------
function get_next_free_face(grid,errcode)
!        ------------------

!----------------------------------------------------
! This routine returns the next available local ID for a face.
! For comments, see get_next_free_vert.
!----------------------------------------------------
...
errcode = 0
get_next_free_face = grid%next_free_face
if (grid%next_free_face > size(grid%face)) then
   call more_faces(grid,errcode)
   if (errcode /= 0) return
endif
if (grid%next_free_face > grid%biggest_face) then
   grid%next_free_face = grid%next_free_face + 1
else
   grid%next_free_face = grid%face(grid%next_free_face)%next
endif
if (get_next_free_face > grid%biggest_face) then
   grid%biggest_face = get_next_free_face
endif

end function get_next_free_face
...
!          ------------------
subroutine put_next_free_face(grid,face)
!          ------------------

!----------------------------------------------------
! This routine adds face to the list of available face IDs
!----------------------------------------------------
...
! add face to the beginning of the linked list of free space

grid%face(face)%next = grid%next_free_face
grid%next_free_face = face

end subroutine put_next_free_face
...
!          ----------
subroutine more_faces(grid,errcode,size_wanted)
!          ----------

!----------------------------------------------------
! This routine increases the space for the number of faces
!----------------------------------------------------
...
type(face_t), pointer :: temp_face(:)
integer, pointer :: temp_face_type(:,:)
integer :: allocstat, oldsize, newsize, i
!----------------------------------------------------
! Begin executable code

oldsize = size(grid%face)
if (present(size_wanted)) then
   newsize = size_wanted
else
   newsize = int(1.5*oldsize)
...
errcode = 0
temp_face => grid%face
temp_face_type => grid%face_type
nullify(grid%face,grid%face_type)
allocate(grid%face(newsize),grid%face_type(newsize,grid%system_size), &
         stat=allocstat)
if (allocstat /= 0) then
   errcode = 1
   call fatal("increased allocation failed")
   grid%face => temp_face
   grid%face_type => temp_face_type
   return
endif
grid%face(1:oldsize) = temp_face
grid%face_type(1:oldsize,:) = temp_face_type

deallocate(temp_face,temp_face_type,stat=allocstat)
grid%next_free_face = oldsize+1

!call count_grid_memory(grid)

end subroutine more_faces
...
! TEMP 3D
call fatal("have not finished 3D version of count_grid_memory")
stop
...
if (associated(grid%eigen_results%eigensolver_l2_resid)) mem = mem + &
   rsize*size(grid%eigen_results%eigensolver_l2_resid)
...
! TEMP 3D
call fatal("have not finished 3D version of phier2nodal")
stop
...
! TEMP 3D
call fatal("have not finished 3D version of nodal2phier")
stop
...
! TEMP 3D
call fatal("have not finished 3D version of nodal2hhier")
stop
...
do i=1,EDGES_PER_ELEMENT
   ...
   element_diameter = max(element_diameter, &
      sqrt((grid%vertex(v1)%coord%x - grid%vertex(v2)%coord%x)**2 + &
           ...
           (grid%vertex(v1)%coord%z - grid%vertex(v2)%coord%z)**2))
end do
...
!----------------------------------------------------
! This routine computes the volume of tetrahedral element elem
!----------------------------------------------------
...
do i=1,3
   A(1,i) = grid%vertex(grid%element(elem)%vertex(i))%coord%x - &
            grid%vertex(grid%element(elem)%vertex(4))%coord%x
   A(2,i) = grid%vertex(grid%element(elem)%vertex(i))%coord%y - &
            grid%vertex(grid%element(elem)%vertex(4))%coord%y
   A(3,i) = grid%vertex(grid%element(elem)%vertex(i))%coord%z - &
            grid%vertex(grid%element(elem)%vertex(4))%coord%z
end do

element_volume = abs(A(1,1)*(A(2,2)*A(3,3)-A(3,2)*A(2,3)) - &
...
! TEMP 3D
if (loc_unowned_neigh .or. loc_owned_neigh .or. loc_bound_vert) then
   call fatal("list_elements: need to check usage of unowned_neigh, owned_neigh and bound_vert in 3D")
   stop
endif
...
element_dof = ((deg-1)*(deg-2)*(deg-3))/6
...
!----------------------------------------------------
! This routine returns the number of degrees of freedom on a face of degree deg
!----------------------------------------------------
...
logical :: only_owned
integer :: lev, vert, elem, deg, edge, face, ivert, iedge, iface
!----------------------------------------------------
! Begin executable code

! TEMP 3D
call fatal("have not finished 3D version of count_dof")
stop
...
! the dofs on the edges and faces that have not yet been visited
...
do lev=1,grid%nlev
   ...
   do while (elem /= END_OF_LIST)
      if (grid%element(elem)%isleaf .and. &
         ...
         deg = grid%element(elem)%degree
         if (deg >= 3) then
            count_dof = count_dof + element_dof(deg)
         endif
         ...
         do iface=1,FACES_PER_ELEMENT
            ...
            if (grid%element(grid%face(face)%assoc_elem)%iown .or. &
                 .not. only_owned) then
               count_dof = count_dof + face_dof(grid%face(face)%degree)* &
                           count(grid%face_type(face,:)/=PERIODIC_SLAVE)
            endif
         end do
      endif
...
!----------------------------------------------------
! This routine returns the z coordinate of pt
!----------------------------------------------------
...
!----------------------------------------------------
! This routine sets the z coordinate of pt to val
!----------------------------------------------------

!----------------------------------------------------
! Dummy arguments

type(point), intent(inout) :: pt
...
real(my_real), intent(out) :: cxx(:,:),cyy(:,:),czz(:,:),cxy(:,:),cxz(:,:), &
                              cyz(:,:),cx(:,:),cy(:,:),cz(:,:),c(:,:),rs(:)
secret_elem = elem
call pdecoefs(x,y,z,cxx,cyy,czz,cxy,cxz,cyz,cx,cy,cz,c,rs)
...
integer, intent(out) :: itype(:)
real(my_real), intent(out) :: c(:,:),rs(:)
call bconds(x,y,z,bmark,itype,c,rs)
...
my_iconds = iconds(x,y,z,comp,eigen)
...
my_trues = trues(x,y,z,comp,eigen)
...
my_truexs = truexs(x,y,z,comp,eigen)
...
my_trueys = trueys(x,y,z,comp,eigen)
...
my_truezs = truezs(x,y,z,comp,eigen)
...
my_phaml_integral_kernel = phaml_integral_kernel(kernel,x,y,z)
...
my_regularity = regularity(x,y,z)
